...
- Q: How should the conversion handle container image references in the output app.bicep? → A: Bicep parameters with `IMAGE_PLACEHOLDER` as the default value, overridable at deploy time via `rad deploy --parameters`.
...
- **Image name/tag mismatch**: Local development may use `project-name:latest` or a project reference while deployment artifacts reference a registry path. The conversion emits Bicep parameters with `IMAGE_PLACEHOLDER` as the default, overridable at deploy time via `rad deploy --parameters`. The developer must set actual image references at deploy time. The mapping report documents this assumption.
...
- **FR-006**: The conversion MUST emit a Bicep `param` declaration for each service's container image, using `IMAGE_PLACEHOLDER` as the default value (e.g., `param apiserviceImage string = 'IMAGE_PLACEHOLDER'`). The Container resource MUST reference this parameter for its `container.image` property, allowing the developer to override the image at deploy time via `rad deploy --parameters`.
...
- **Service**: A compute workload in the application (e.g., webfrontend, apiservice). In Aspire, represented as a project resource with `AddProject<>()` in `AppHost.cs` and a corresponding `.tmpl.yaml` template. In Radius, represented as a `Applications.Core/containers` resource.
...
- Service container images are assumed to be pre-built and available in a local or accessible registry. The conversion does not build images. Image references are emitted as Bicep parameters defaulting to `IMAGE_PLACEHOLDER`, overridable at deploy time via `rad deploy --parameters`.
