...
- Q: Which Aspire resource types should be supported beyond containers? → A: Containers + known backing-service types (e.g., Redis, PostgreSQL) mapped to Radius data resources. Secret parameters (`parameter.v0` with `secret: true`) are mapped to `@secure()` Bicep parameters. `annotated.string` resources with `filter: "uri"` generate best-effort `uriComponent()` variables. All other types emit warnings.
...
- Q: What is explicitly out of scope for v1? → A: Cloud resource provisioning (Azure/AWS), Dapr integration configuration, and service discovery are all out of scope. Secret parameter handling (`parameter.v0` with `secret: true`) IS in scope — these are mapped to `@secure()` Bicep parameters.
...
5. **Given** a valid Aspire manifest containing a `parameter.v0` resource with `secret: true` inputs, **When** the conversion command runs, **Then** the generated Bicep file includes an `@secure()` parameter declaration for that secret, and all environment variable references to the parameter value (e.g., `{cache-password.value}`) are resolved to Bicep parameter references.
6. **Given** a valid Aspire manifest containing an `annotated.string` resource with `filter: "uri"` referencing a parameter, **When** the conversion command runs, **Then** the generated Bicep file includes a variable declaration using `uriComponent()` that wraps the referenced parameter, and environment variable references to the annotated string are resolved to Bicep string interpolation expressions using that variable.
...
- **FR-004**: The command MUST resolve Aspire expression references in environment variables and command arguments, converting them to appropriate Bicep constructs:
  - `{resource.bindings.name.host}` → the resource's name as a string literal (e.g., `'cache'`)
  - `{resource.bindings.name.port}` and `{resource.bindings.name.targetPort}` → the port number as a string literal (e.g., `'6379'`); self-references (same resource) resolve to the literal value
  - `{param.value}` where `param` is a `parameter.v0` → a Bicep parameter reference (e.g., `cache_password`)
  - `{annotated.value}` where `annotated` is an `annotated.string` → a Bicep variable reference (e.g., `cache_password_uri_encoded`)
  - `{resource.connectionString}` → fully expanded by recursively resolving the resource's connection string template
  - Composite expressions with embedded references MUST use Bicep string interpolation (e.g., `'redis://:${cache_password_uri_encoded}@cache:6379'`)
- **FR-005**: The command MUST generate a top-level Radius application resource and wire all container resources to it.
- **FR-007**: The command MUST generate `extension radius` at the top of the output file. This single extension is sufficient for all Radius resource types (`Radius.Core/applications`, `Radius.Compute/containers`, `Radius.Compute/routes`).
...
- **FR-020**: The command MUST map Aspire `parameter.v0` resources whose inputs include `secret: true` to Bicep `@secure()` parameter declarations. The parameter name MUST be derived from the Aspire resource name (with hyphens converted to underscores for valid Bicep identifiers). Environment variable references to the parameter value (e.g., `{cache-password.value}`) MUST resolve to the Bicep parameter name. Non-secret `parameter.v0` resources remain unsupported and emit warnings per FR-013.
- **FR-021**: The command MUST map Aspire `annotated.string` resources with `filter: "uri"` to Bicep variable declarations using the `uriComponent()` function applied to the referenced source value. For example, an `annotated.string` with `value: "{cache-password.value}"` and `filter: "uri"` MUST generate `var cache_password_uri_encoded = uriComponent(cache_password)`. A descriptive `@description()` decorator SHOULD be emitted on the variable. The `annotated.string` resource type MUST still include an unsupported comment in the generated Bicep (since only the `uri` filter is handled as a best-effort mapping), but environment variable references to the annotated string value MUST resolve to the Bicep variable.

**Resource Exclusion Priority**: When evaluating a resource, the command MUST apply exclusion checks in the following order: (1) error field present → skip per FR-018, (2) `build.buildOnly: true` → skip per FR-019, (3) `parameter.v0` with `secret: true` → map to @secure() param per FR-020, (4) `annotated.string` with `filter: "uri"` → map to uriComponent() variable per FR-021, (5) unsupported type → warn per FR-013, (6) supported type → map normally.
### Key Entities
...
- `annotated.string` resource types with `filter: "uri"` are handled as a best-effort mapping: a `var` declaration using `uriComponent()` is generated. Other filter types are treated as unsupported. The generated variable enables correct Bicep string interpolation for environment variables that reference URI-encoded values.
...
- **Non-secret parameter handling**: Aspire `parameter.v0` resources without `secret: true` inputs are not mapped to Bicep parameters. These are treated as unsupported resource types and emit warnings. Users must manually add any required Bicep parameters for non-secret values after conversion. Note: secret parameters (`parameter.v0` with `secret: true`) ARE handled — they are mapped to `@secure()` Bicep parameters per FR-020.
